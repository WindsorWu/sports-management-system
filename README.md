...
- **后台系统**：数据大屏（包含**实时评论词云**）、用户管理、赛事管理、报名审核、成绩录入与导入、公告管理、轮播图及反馈管理。
...
6. 启动词云服务（WebSocket）：`daphne sports_backend.asgi:application --port 8090`。
...
- **互动体验**: 发表评论、点赞收藏、**实时评论词云分析**（基于 WebSocket）。
